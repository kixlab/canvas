...
def load_datasets_singleduration(
    dataset, bp="/netpool/work/gpu-2/users/wangyo/datasets", return_test=False
):
    fix_as_mat = False
    fix_key = None
    ...
    if dataset == "gdi":
        ...
        img_filenames_train = sorted(
            [os.path.join(img_path_train, f) for f in os.listdir(img_path_train)]
        )
        imp_filenames_train = sorted(
            [os.path.join(imp_path_train, f) for f in os.listdir(imp_path_train)]
        )
        img_filenames_val = sorted(
            [os.path.join(img_path_val, f) for f in os.listdir(img_path_val)]
        )
        imp_filenames_val = sorted(
            [os.path.join(imp_path_val, f) for f in os.listdir(imp_path_val)]
        )
        ...
        img_filenames_test = sorted(
            [os.path.join(img_path_test, f) for f in os.listdir(img_path_test)]
        )

        fix_filenames_train = sorted(
            [
                os.path.join(fix_path_train, f)
                for f in os.listdir(fix_path_train)
                if f.endswith(".png")
            ]
        )
        fix_filenames_val = sorted(
            [
                os.path.join(fix_path_val, f)
                for f in os.listdir(fix_path_val)
                if f.endswith(".png")
            ]
        )

        fixcoords_filenames_train = sorted(
            [
                os.path.join(fixcoords_path_train, f)
                for f in os.listdir(fixcoords_path_train)
            ]
        )
        fixcoords_filenames_val = sorted(
            [
                os.path.join(fixcoords_path_val, f)
                for f in os.listdir(fixcoords_path_val)
            ]
        )
        ...
        maps = sorted(
            [
                os.path.join(imp_path, f)
                for f in os.listdir(imp_path)
                if "fixMap" in f and f.endswith(".jpg")
            ]
        )
        fixs = sorted(
            [
                os.path.join(imp_path, f)
                for f in os.listdir(imp_path)
                if "fixPts" in f and f.endswith(".jpg")
            ]
        )
        ...
        if not use_tts_file:
            for f in os.listdir(img_path):
                ...
                imgs = sorted(
                    [
                        os.path.join(img_path, f, i)
                        for i in os.listdir(os.path.join(img_path, f))
                        if i.endswith((".png", ".jpg"))
                    ]
                )
                maps = sorted(
                    [
                        os.path.join(imp_path, f, i)
                        for i in os.listdir(os.path.join(imp_path, f))
                        if i.endswith((".png", ".jpg"))
                    ]
                )
                ...
                img_filenames_train = np.concatenate(
                    [img_filenames_train, imgs[:-k]], axis=None
                )
                img_filenames_val = np.concatenate(
                    [img_filenames_val, imgs[-k:]], axis=None
                )
                imp_filenames_train = np.concatenate(
                    [imp_filenames_train, maps[:-k]], axis=None
                )
                imp_filenames_val = np.concatenate(
                    [imp_filenames_val, maps[-k:]], axis=None
                )

        else:
            with open(
                os.path.join(bp, "imp1k", "train_test_split_imp1k.json"), "r"
            ) as f:
                tt_s = json.load(f)
        ...
        for f in os.listdir(img_path):
            ...
            fixs = sorted(
                [
                    os.path.join(fix_path, f, i)
                    for i in os.listdir(os.path.join(fix_path, f))
                    if i.endswith(".mat")
                ]
            )
            ...
            img_filenames_train = np.concatenate(
                [img_filenames_train, imgs[:-10]], axis=None
            )
            img_filenames_val = np.concatenate(
                [img_filenames_val, imgs[-10:]], axis=None
            )
            imp_filenames_train = np.concatenate(
                [imp_filenames_train, maps[:-10]], axis=None
            )
            imp_filenames_val = np.concatenate(
                [imp_filenames_val, maps[-10:]], axis=None
            )
            fix_filenames_train = np.concatenate(
                [fix_filenames_train, fixs[:-10]], axis=None
            )
            fix_filenames_val = np.concatenate(
                [fix_filenames_val, fixs[-10:]], axis=None
            )

        for f in os.listdir(img_path_test):
            new_files = sorted(
                [
                    os.path.join(img_path_test, f, i)
                    for i in os.listdir(os.path.join(img_path_test, f))
                    if i.endswith(".jpg")
                ]
            )
            img_filenames_test = np.concatenate(
                [img_filenames_test, new_files], axis=None
            )
...
def load_multiduration_data(
    img_folder, map_folder, fix_folder, times=[500, 3000, 5000]
):
    """
    ...
    img_filenames = sorted(
        [
            ...
            if f.endswith(".jpg") or f.endswith(".png") or f.endswith(".npy")
        ]
    )
    ...
    for t in avlb_times:
        if t in times:
            ...
            t_map_filenames = sorted(
                [
                    os.path.join(map_folder, str(t), f)
                    for f in os.listdir(os.path.join(map_folder, str(t)))
                    if f.endswith(".png") or f.endswith(".npy")
                ]
            )
            t_fix_filenames = sorted(
                [
                    os.path.join(fix_folder, str(t), f)
                    for f in os.listdir(os.path.join(fix_folder, str(t)))
                    if f.endswith(".png") or f.endswith(".npy")
                ]
            )
...
def load_datasets_multiduration(
    dataset, times, bp="/netpool/work/gpu-2/users/wangyo/datasets", test_splits=[0]
):
    fix_as_mat = False
    fix_key = ""
    uses_fix = True
    ...
    if dataset == "salicon":
        ...
        img_files_train = sorted(
            [os.path.join(img_path_train, f) for f in os.listdir(img_path_train)]
        )
        map_files_train = [
            sorted(
                [os.path.join(map_path_train, f) for f in os.listdir(map_path_train)]
            )
        ] * rep
        fix_files_train = [
            sorted(
        ...
        img_files_val = sorted(
            [os.path.join(img_path_val, f) for f in os.listdir(img_path_val)]
        )
        map_files_val = [
            sorted([os.path.join(map_path_val, f) for f in os.listdir(map_path_val)])
        ] * rep
        fix_files_val = [
            sorted(
                [
        ...
        img_files, map_files, fix_files = load_multiduration_data(
            img_path, map_path, fix_path, times=times
        )
        ...
        raise ValueError(
            "Unknown dataset. Possible values are codecharts, salicon or salicon_md"
        )
    ...
    print(
        "map_files_test: ",
        [len(elt) for elt in map_files_test] if map_files_test else map_files_test,
    )
    ...
    print(
        "fix_files_test: ",
        [len(elt) for elt in fix_files_test] if fix_files_test else fix_files_test,
    )
