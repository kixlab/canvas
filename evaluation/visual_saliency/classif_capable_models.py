...
def xception_cl_fus(
    ...
    if print_shapes:
        print("aspp out shapes:", aspp_out.shape)

    ### FUSION ###
    dense_f = Dense(internal_filts, name="dense_fusion")(g_n)
    if print_shapes:
        print("dense_f shapes:", dense_f.shape)
    reshap = Lambda(
        ...
        lambda s: (s[0], K.int_shape(aspp_out)[1], K.int_shape(aspp_out)[2], s[1]),
    )(dense_f)
    if print_shapes:
        print("after lambda shapes:", reshap.shape)

    conc = Concatenate()([aspp_out, reshap])

    ### Projection ###
    x = Conv2D(
        internal_filts, (1, 1), padding="same", use_bias=False, name="concat_projection"
    )(conc)
    ...
    ### DECODER ###
    outs_dec = decoder_block(
        x, dil_rate=dil_rate, print_shapes=print_shapes, dec_filt=internal_filts, dp=dp
    )
...
def xception_cl_fus_aspp(
    ...
    if not lambda_layer_for_save:
        ...
        reshap = Lambda(
            lambda x: K.repeat_elements(
                K.expand_dims(
                    K.repeat_elements(K.expand_dims(x, axis=1), 40, axis=1), axis=1
                ),
                30,
                axis=1,
            ),
            lambda s: (s[0], 30, 40, s[1]),
        )(dense_f)
    ...
    # Building model
    m = Model(
        inp, outs_final, name="xception_cl_fus_aspp"
    )  # Last element of outs_final is classification vector
...
def umsi(
    ...
    outs_dec = decoder_with_skip(
        x, skip_feature_maps, print_shapes=print_shapes, dec_filt=internal_filts, dp=dp
    )
...
def global_net(x, nfilts=512, dp=0.1, print_shapes=True):
    x = Conv2D(
        nfilts, (3, 3), strides=3, padding="same", use_bias=False, name="global_conv"
    )(x)
    if print_shapes:
        print("Shape after global net conv:", x.shape)
...
def app(x, nfilts=256, prefix="app", dils=[6, 12, 18]):
    ...
    x2 = Conv2D(
        ...
        padding="same",
        activation="relu",
        dilation_rate=(dils[0], dils[0]),
        name=prefix + "_c3x3d" + str(dils[0]),
    )(x)
    x3 = Conv2D(
        nfilts,
        3,
        padding="same",
        activation="relu",
        dilation_rate=(dils[1], dils[1]),
        name=prefix + "_c3x3d" + str(dils[1]),
    )(x)
    x4 = Conv2D(
        nfilts,
        3,
        padding="same",
        activation="relu",
        dilation_rate=(dils[2], dils[2]),
        name=prefix + "_c3x3d" + str(dils[2]),
    )(x)
...
def aspp(x, nfilts=256, prefix="aspp", dils=[6, 12, 18]):
    x1 = Conv2D(nfilts, (1, 1), padding="same", use_bias=False, name=prefix + "_csep0")(
        x
    )
...
def decoder_with_skip(
    ...
    for i, sk in enumerate(skip_tensors, start=1):
        # Upsample
        x = UpSampling2D((2, 2), interpolation="bilinear", name=prefix + "_ups%d" % i)(
            x
        )
        if x.shape[1] != sk.shape[1] or x.shape[2] != sk.shape[2]:
            x = Lambda(
                lambda t: tf.image.resize(t, (K.int_shape(sk)[1], K.int_shape(sk)[2]))
            )(x)
    ...
    if print_shapes:
        print("Shape after last ups and resize:", x.shape)

    # Final conv to get to a heatmap
    x = Conv2D(
        1, kernel_size=1, padding="same", activation="relu", name=prefix + "_c_out"
    )(x)
    if print_shapes:
...
def decoder_block(
    x, dil_rate=(2, 2), print_shapes=True, dec_filt=1024, dp=0.2, ups=16, prefix="dec"
):
