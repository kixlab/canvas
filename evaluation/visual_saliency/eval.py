...
def sim_npy(gt, predicted):
    ...
    predicted = (predicted - np.min(predicted)) / (
        np.max(predicted) - np.min(predicted)
    )
    predicted = predicted / np.sum(predicted)
...
def cc_match_npy(gt, pred):
    ...
    for t in range(len(gt) - 1):
        ccm += np.abs(
            np.corrcoef(gt[t].reshape(-1), gt[t + 1].reshape(-1))[0, 1]
            - np.corrcoef(pred[t].reshape(-1), pred[t + 1].reshape(-1))[0, 1]
        )
    return ccm / (len(gt) - 1)
...
def predict_and_save(
    ...
    for imfile in tqdm.tqdm(test_img):
        ...
        p = postprocess_predictions(
            p, gt_shape[0], gt_shape[1], blur, normalize=False, zero_to_255=True
        )
    ...
    for imfile in tqdm.tqdm(test_img):
        ...
        for time, timelen in enumerate(times):
            if mode == "multistream_concat":
                ...
                raise ValueError("Unknown mode")
            p = postprocess_predictions(
                p, gt_shape[0], gt_shape[1], blur, normalize=False
            )
            p_norm = (p - np.min(p)) / (np.max(p) - np.min(p))
            p_img = p_norm * 255
            hm_img = Image.fromarray(np.uint8(p_img), "L")
            ...
            imname = (
                os.path.splitext(os.path.basename(imfile))[0]
                + "_"
                + str(timelen)
                + "."
                + ext
...
def calculate_metrics(
    p, gt_map=None, gt_fix_map=None, gt_fix_points=None, gt_labels=None, p_labels=None
):
    ...
    p_norm = (p - np.min(p)) / (np.max(p) - np.min(p))
    if np.max(gt_map) > 1:
        gt_map = np.array(gt_map, dtype=np.float32) / 255.0
...
def get_stats_multiduration(
    ...
    m_per_dataset_by_time = {
        d: {t: {} for t in range(n_times)} for d in CODECHARTS_SUB_DATASETS
    }
    batch = 0
    top_n = {}
    bot_n = {}
    ...
    for dat in tqdm.tqdm_notebook(gen):
        ...
        for batch_idx, prediction in enumerate(pred_batch):
            ...
            for t in range(n_times):
                ...
                p_t = postprocess_predictions(
                    p_t, gt_map_t.shape[0], gt_map_t.shape[1], blur, normalize=False
                )

                p_t_dict[t] = (p_t - np.min(p_t)) / (np.max(p_t) - np.min(p_t))

                gt_fix_points_t = (
                    gt_fix_points_batch[batch_idx][t] if gt_fix_points_batch else None
                )
                assert p_t.shape == gt_map_t.shape, (
                    "prediction and ground truth should have same dimensions, but got %s and %s"
                    % (p_t.shape, gt_map_t.shape)
                )
                ...
                for k, v in m.items():
                    ...
                    m_by_time[t][k] = (
                        m_by_time[t].get(k, []) + v
                    )  # append list to list or int to int
                    if get_stats_per_dataset:
                        m_per_dataset[name_to_dataset(names_batch[batch_idx])][k] = (
                            m_per_dataset[name_to_dataset(names_batch[batch_idx])].get(
                                k, []
                            )
                            + v
                        )
                        m_per_dataset_by_time[name_to_dataset(names_batch[batch_idx])][
                            t
                        ][k] = (
                            m_per_dataset_by_time[
                                name_to_dataset(names_batch[batch_idx])
                            ][t].get(k, [])
                            + v
                        )
            ...
            # Calculate CC Match
            all_m["CCM"] = all_m.get("CCM", []) + [
                cc_match_npy(gt_map_batch[batch_idx], prediction)
            ]
            ...
            if compare_across_times:
                for t_lower, others in combos.items():
                    for t_higher in others.keys():
                        combos[t_lower][t_higher].append(
                            cc_npy(p_t_dict[t_lower], p_t_dict[t_higher])
                        )
    ...
    if return_top_bot_n:
        for me in metrics_for_top_bot_n:
            ...
            if len(top_n.get(me, [])) < return_top_bot_n:
                item = (
                    np.mean(all_m[me][-n_times:]),
                    idx,
                    mt,
                    imgs[0],
                    hms_by_time,
                    gt_map_batch,
                    [combos[0][1][-1], combos[0][2][-1], combos[1][2][-1]],
                )
...
def get_stats_oneduration(
    ...
    for dat in tqdm.tqdm_notebook(gen):
        ...
        for i in range(len(pred_batch)):  # loop over batch
            ...
            p = postprocess_predictions(
                p, gt_size[0], gt_size[1], blur, normalize=False
            )
            ...
            assert p.shape == gt_map.shape, (
                "prediction and ground truth should have same dimensions, but are %s and %s"
                % (p.shape, gt_map.shape)
            )
...
def log_metrics(
    ...
    with open(txt_path, "a+") as f:
        ...
        f.write("\nCkpt: %s" % ckpt)
        f.write(
            "\nEvaluated on %d elems of %s with fct %s and blur=%s on: %s"
            % (n_elems, dataset, eval_with, blur, str(datetime.datetime.now()))
        )
        if print_:
            print("Ckpt:", ckpt)
        ...
        if simple_log:
            ...
            if compare_across_times:
                ...
                for t_lower, others in combos.items():
                    for t_higher, v in others.items():
                        f.write(
                            "\nCC for times %d and %d: %.4f"
                            % (t_lower, t_higher, np.mean(v))
                        )
