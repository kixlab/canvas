...
def kl_time(y_true, y_pred):
    ...
    max_y_pred = K.repeat_elements(
        K.expand_dims(
            K.repeat_elements(
                K.expand_dims(K.max(K.max(y_pred, axis=ax), axis=ax), axis=ax),
                shape_r_out,
                axis=ax,
            ),
            axis=ax + 1,
    ...
    sum_y_true = K.repeat_elements(
        K.expand_dims(
            K.repeat_elements(
                K.expand_dims(K.sum(K.sum(y_true, axis=ax), axis=ax), axis=ax),
                shape_r_out,
                axis=ax,
            ),
            axis=ax + 1,
    ...
    sum_y_pred = K.repeat_elements(
        K.expand_dims(
            K.repeat_elements(
                K.expand_dims(K.sum(K.sum(y_pred, axis=ax), axis=ax), axis=ax),
                shape_r_out,
                axis=ax,
            ),
            axis=ax + 1,
    ...
    kl_out = K.sum(
        K.sum(y_true * K.log((y_true / (y_pred + K.epsilon())) + K.epsilon()), axis=ax),
        axis=ax,
    )
...
def nss_time(y_true, y_pred):
    ...
    max_y_pred = K.repeat_elements(
        K.expand_dims(first_rep, axis=ax + 1), shape_c_out, axis=ax + 1
    )
    y_pred /= max_y_pred

    if len(y_true.shape) == 5:
        y_pred_flatten = K.reshape(
            y_pred,
            (
                K.shape(y_pred)[0],
                K.shape(y_pred)[1],
                K.shape(y_pred)[2] * K.shape(y_pred)[3] * K.shape(y_pred)[4],
            ),
    ...
    y_mean = K.repeat_elements(
        K.expand_dims(
            K.repeat_elements(
                K.expand_dims(K.expand_dims(y_mean)), shape_r_out, axis=ax
            )
        ),
        shape_c_out,
        axis=ax + 1,
    ...
    y_std = K.repeat_elements(
        K.expand_dims(
            K.repeat_elements(K.expand_dims(K.expand_dims(y_std)), shape_r_out, axis=ax)
        ),
        shape_c_out,
        axis=ax + 1,
    )
...
def nss(y_true, y_pred):
    ...
    max_y_pred = K.repeat_elements(
        K.expand_dims(
            K.repeat_elements(
                K.expand_dims(K.max(K.max(y_pred, axis=ax), axis=ax), axis=ax + 1),
                shape_r_out,
                axis=ax,
            ),
            axis=ax + 1,
