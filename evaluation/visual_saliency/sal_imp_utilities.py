...
def preprocess_fixmaps(paths, shape_r, shape_c, fix_as_mat=False, fix_key="", pad=True):
    ...
    for i, path in enumerate(paths):
        ...
        if DEBUG:
            print(
                "fix_map shape, np.max(fix_map),np.min(fix_map),np.mean(fix_map)",
                fix_map.shape,
                np.max(fix_map),
                np.min(fix_map),
                np.mean(fix_map),
        ...
        if pad:
            ims[i, :, :, 0] = padding_fixation(
                fix_map, shape_r=shape_r, shape_c=shape_c
            )
        else:
            ims.append(fix_map)
    #             ims = np.array(ims)
...
def postprocess_predictions(
    pred, shape_r, shape_c, blur=False, normalize=False, zero_to_255=False
):
...
class MultidurationGenerator(Sequence):
    def __init__(
        ...
        print(
            "Instantiating MultidurationGenerator. \
        ...
        if (mode == "multistream_concat") and (
            map_filenames is None or fix_filenames is None
        ):
            print(
                "Multistream concat can only be used when both fixations and maps are provided. \
        ...
        if map_filenames is not None:
            ...
            assert all(
                [len(self.img_filenames) == len(elt) for elt in self.map_filenames]
            ), (
                "Mismatch between images and maps. Images size: "
                + self.img_filenames.shape.__str__()
                + " Maps size: "
                + self.map_filenames.shape.__str__()
            )
            ...
            print(
                "Warning: No maps filenames provided, no outputs of that kind will be generated"
            )
        ...
        if fix_filenames is not None:
            ...
            assert all(
                [len(self.img_filenames) == len(elt) for elt in self.fix_filenames]
            ), (
                "Mismatch between images and fixations. Images size: "
                + self.img_filenames.shape.__str__()
                + " Fix size: "
                + self.fix_filenames.shape.__str__()
            )
            ...
            print(
                "Warning: No fix filenames provided, no outputs of that kind will be generated"
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.n_output_maps >= 1:
            ...
            for t in range(self.timesteps):
                maps_names_t = self.map_filenames[t][
                    idx * self.batch_size : (idx + 1) * self.batch_size
                ]
        ...
        if self.n_output_fixs >= 1:
            fixs = []
            for t in range(self.timesteps):
                fix_names_t = self.fix_filenames[t][
                    idx * self.batch_size : (idx + 1) * self.batch_size
                ]
        ...
        if self.mode == "singlestream":
            ...
            for i in range(self.timesteps):
                outs.append(
                    np.concatenate(
                        [
                            np.expand_dims(maps[i], axis=1),
                            np.expand_dims(fixs[i], axis=1),
                        ],
                        axis=1,
                    )
                )
...
class SalImpGenerator(Sequence):
    def __init__(
        ...
        print(
            "Instantiating SalImpGenerator. Number of files received: %d. Batch size: %d. Image size: %s. Map size: %s. Augmentation: %d, Pad_imgs: %s. Pad_maps: %s."
            % (
                len(img_filenames),
                batch_size,
                str(img_size),
    ...
    def __getitem__(self, idx):
        batch_x = self.img_filenames[
            idx * self.batch_size : (idx + 1) * self.batch_size
        ]
        batch_y = self.imp_filenames[
            idx * self.batch_size : (idx + 1) * self.batch_size
        ]
        ...
        if self.read_npy:
            ...
            images = preprocess_images(
                batch_x, self.img_size[0], self.img_size[1], pad=self.pad_imgs
            )
            maps = preprocess_maps(
                batch_y, self.map_size[0], self.map_size[1], pad=self.pad_maps
            )
        ...
        if self.fix_filenames is not None and self.concat_fix_and_maps:
            outs = np.concatenate(
                [np.expand_dims(maps, axis=1), np.expand_dims(fixs, axis=1)], axis=1
            )
            if self.n_output_maps > 1:
        ...
        for t in range(n_times):
            ...
            if fixcoord_filenames:
                ...
                if fixcoord_filetype == "mat":
                    ...
                    raise RuntimeError(
                        "fixcoord filetype %s is unsupported" % fixcoord_filetype
                    )
            else:
                all_fixations = None
            imgs.append(img)
...
class ImpAndClassifGenerator(Sequence):
    def __init__(
        ...
        print(
            "Instantiating ImpAndClassifGenerator. Number of files received: %d. Extras: %s. Batch size: %d. Image size: %s. Map size: %s. Augmentation: %d, Pad_imgs: %s. Pad_maps: %s. Num classes: %d."
            % (
                len(img_filenames),
                len(extra_imgs) if extra_imgs is not None else None,
                batch_size,
                str(img_size),
                str(map_size),
                augment,
    ...
    def __getitem__(self, idx):
        batch_x = self.imgs_this_epoch[
            idx * self.batch_size : (idx + 1) * self.batch_size
        ]
        batch_y = self.imps_this_epoch[
            idx * self.batch_size : (idx + 1) * self.batch_size
        ]
        ...
        if not self.dummy_labels:
            labels = get_labels(
                batch_x, self.category_dict, self.str2label
            )  # Returns a numpy array of shape (bs, num_classes)
        else:
            labels = np.zeros((len(images), self.num_classes))
        if self.fix_filenames is not None:
            if read_npy:
                fixs = load_images(
                    self.fixs_this_epoch[
                        idx * self.batch_size : (idx + 1) * self.batch_size
                    ]
                )
            else:
    ...
    def on_epoch_end(self):
        ...
        self.imgs_this_epoch = np.concatenate(
            [self.img_filenames, extra_imgs_this_epoch]
        )
        self.imps_this_epoch = np.concatenate(
            [self.imp_filenames, extra_imps_this_epoch]
        )

        if self.fix_filenames is not None:
            self.fixs_this_epoch = np.concatenate(
                [self.fix_filenames, extra_fixs_this_epoch]
            )
