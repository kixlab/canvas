...
def decoder_block(x, dil_rate=(2, 2), print_shapes=True, dec_filt=1024):
    ...
    x = Conv2D(
        dec_filt // 2, 3, padding="same", activation="relu", dilation_rate=(2, 2)
    )(x)
    x = Conv2D(
        dec_filt // 2, 3, padding="same", activation="relu", dilation_rate=(2, 2)
    )(x)
    ...
    x = Conv2D(
        dec_filt // 4, 3, padding="same", activation="relu", dilation_rate=(2, 2)
    )(x)
    x = Conv2D(
        dec_filt // 4, 3, padding="same", activation="relu", dilation_rate=(2, 2)
    )(x)
...
def decoder_block_dp(x, dil_rate=(2, 2), print_shapes=True, dec_filt=1024, dp=0.3):
    # Dilated convolutions
    x = Conv2D(dec_filt, 3, padding="same", activation="relu", dilation_rate=dil_rate)(
        x
    )
    x = Conv2D(dec_filt, 3, padding="same", activation="relu", dilation_rate=dil_rate)(
        x
    ...
    x = Conv2D(
        dec_filt // 2, 3, padding="same", activation="relu", dilation_rate=dil_rate
    )(x)
    x = Conv2D(
        dec_filt // 2, 3, padding="same", activation="relu", dilation_rate=dil_rate
    )(x)
    ...
    x = Conv2D(
        dec_filt // 4, 3, padding="same", activation="relu", dilation_rate=dil_rate
    )(x)
    x = Dropout(dp)(x)
    x = UpSampling2D((4, 4), interpolation="bilinear")(x)
...
def xception_decoder(
    ...
    ## DECODER ##
    outs_dec = decoder_block(
        xception.output, dil_rate=dil_rate, print_shapes=print_shapes, dec_filt=512
    )
...
def resnet_decoder(
    ...
    if print_shapes:
        print("resnet output shape:", dcn.output.shape)

    ## DECODER ##
    outs_dec = decoder_block(
        dcn.output, dil_rate=dil_rate, print_shapes=print_shapes, dec_filt=512
    )
...
def fcn_vgg16(
    ...
    classif_layer_fc7_ups = UpSampling2D(size=(2, 2), interpolation="bilinear")(
        classif_layer_fc7
    )
    ...
    def concat_one(fc7):
        ...
        return K.concatenate(
            [K.zeros(shape=shape_zeros), classif_layer_fc7_ups], axis=1
        )
...
def sam_simple(
    ...
    x = AttentiveConvLSTM2D(
        filters=512,
        attentive_filters=512,
        kernel_size=(3, 3),
...
def sam_resnet_nopriors(
    ...
    att_convlstm = AttentiveConvLSTM2D(
    ...
    dil_conv1 = Conv2D(
        conv_filters, 5, padding="same", activation="relu", dilation_rate=(4, 4)
    )(att_convlstm)
    dil_conv2 = Conv2D(
        conv_filters, 5, padding="same", activation="relu", dilation_rate=(4, 4)
    )(dil_conv1)
    ...
    m = Model(inp, outs_final)
    if verbose:
        pass  # Suppress model summary output
...
def sam_resnet_new(
    ...
    att_convlstm = AttentiveConvLSTM2D(
        ...
        padding="same",
        return_sequences=False,
        input_shape=att_convlstm.shape,
    )(att_convlstm)
...
def sam_xception_new(
    ...
    inp = Input(shape=input_shape)
    dcn = Xception_wrapper(
        include_top=False, weights="imagenet", input_tensor=inp, pooling=None
    )
    if print_shapes:
        print("xception:", dcn.output.shape)
...
def xception_se_lstm_singledur(
    input_shape=(shape_r, shape_c, 3),
    conv_filters=256,
    lstm_filters=512,
    verbose=True,
    ...
    ### DECODER ###
    outs_dec = decoder_block(
        x, dil_rate=(2, 2), print_shapes=print_shapes, dec_filt=conv_filters
    )
...
def se_lstm_block(
    ...
    inp_rep = Lambda(
        lambda y: K.repeat_elements(K.expand_dims(y, axis=1), nb_timestep, axis=1),
    ...
    x = LSTM(
        lstm_filters, return_sequences=return_sequences, unroll=True, activation="relu"
    )(x)
    if print_shapes:
    ...
    x = Lambda(
        lambda y: K.expand_dims(K.expand_dims(y, axis=1), axis=1),
        lambda s: (s[0], 1, 1, s[-1]),
    )(x)
    if print_shapes:
...
def xception_aspp(
    ...
    # Conv1,2,ASPP
    dil_conv1 = Conv2D(
        conv_filters, 3, padding="same", activation="relu", dilation_rate=(2, 2)
    )(x)
    dil_conv2 = Conv2D(
        conv_filters, 3, padding="same", activation="relu", dilation_rate=(4, 4)
    )(x)
    dil_conv3 = Conv2D(
        conv_filters, 3, padding="same", activation="relu", dilation_rate=(8, 8)
    )(x)
    pass
...
def UMSI(
    ...
    if freeze_enc:
        for layer in xception.layers:
            layer.trainable = False
    # ASPP
    c0 = Conv2D(256, (1, 1), padding="same", use_bias=False, name="aspp_csep0")(
        xception.output
    )
    c6 = DepthwiseConv2D(
        (3, 3),
    ...
    c6 = Conv2D(
        256, (1, 1), padding="same", use_bias=False, name="aspp_csepd6_pointwise"
    )(c6)
    c12 = Conv2D(
        256, (1, 1), padding="same", use_bias=False, name="aspp_csepd12_pointwise"
    )(c12)
    c18 = Conv2D(
        256, (1, 1), padding="same", use_bias=False, name="aspp_csepd18_pointwise"
    )(c18)
    ...
    x = Conv2D(
        256, (3, 3), strides=(3, 3), padding="same", use_bias=False, name="global_conv"
    )(xception.output)
    ...
    classif = Dropout(0.3, name="dropout_2")(x)
