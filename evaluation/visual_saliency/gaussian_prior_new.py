...
class LearningPrior(Layer):
    def __init__(
        ...
        **kwargs
        ...
        if not init:
            ...
            )  # replaced from gaussian_priors_init
        else:
    ...
    def call(self, x):
        ...
        x_t = K.dot(
            K.ones((self.height, 1)),
            K.expand_dims(self._linspace(0, 1.0, self.width), axis=0),
        )
        y_t = K.dot(
            K.expand_dims(self._linspace(e1, e2, self.height), axis=1),
            K.ones((1, self.width)),
        )
        ...
        max_gauss = K.repeat_elements(
            K.expand_dims(
                K.repeat_elements(
                    K.expand_dims(K.max(K.max(gaussian, axis=0), axis=0), axis=0),
                    self.width,
                    axis=0,
                ),
                axis=0,
