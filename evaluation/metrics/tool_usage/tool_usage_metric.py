...
def get_tool_usage_metrics(
    result_path: str, case_id: str, snapshot_num: Optional[int] = None, **kwargs
) -> Dict[str, Any]:
    """
    ...
    for snap_path in snapshot_files:
        ...
        current_step_tools = [
            tool.get("name") for tool in tool_calls_in_step if tool.get("name")
        ]
    ...
    tool_efficiency = round(total_tool_calls / step_count, 4) if step_count > 0 else 0.0
    ...
    if human_tools:
        ...
        human_tool_precision = (
            round(intersection / len(unique_tools), 4) if len(unique_tools) > 0 else 0.0
        )

        human_tool_recall = (
            round(intersection / len(human_tools_set), 4)
            if len(human_tools_set) > 0
            else 0.0
        )
